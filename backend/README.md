...
| Скрипт | Что делает |
| --- | --- |
| `npm run prisma:migrate` | создать миграцию после правки `prisma/schema.prisma` |
...
| PATCH/DELETE | `/admin/categories/:id` | изменить / удалить (нельзя, если есть товары или подкатегории → 409) |
| GET/POST | `/admin/brands` | список / создать |
| PATCH/DELETE | `/admin/brands/:id` | изменить / удалить (у товаров бренд обнуляется) |
| POST | `/admin/uploads` | `multipart/form-data`, поле `file`: JPEG/PNG/WebP/AVIF до 5 МБ → `{ url }` |
| DELETE | `/admin/uploads/:filename` | удалить файл |
...
| PATCH/DELETE | `/admin/users/:id` | изменить / удалить (себя удалить нельзя) |
...
Ошибки: `400` — валидация (лишние поля тоже отклоняются), `401` — нет токена, `403` — не хватает роли, `404` — не найдено, `409` — конфликт (занятый slug/sku, удаление связанной записи).
